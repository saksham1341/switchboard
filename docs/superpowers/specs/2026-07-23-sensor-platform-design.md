...
```python
class HttpServer:
    def __init__(self, host="0.0.0.0", port=8080, *, serve=True): ...
    def route(self, path, handler, *, owner: str, methods=("GET",)) -> None: ...
...
`owner` is diagnostic and never affects routing. It names the component claiming the request, so a collision can say *who* it collided with — "already registered by `'github'`" rather than bare "already registered", which on a server shared by several sensors is the difference between a five-second fix and a grep. It is **required, not defaulted**: an unattributed claim produces exactly the useless message the parameter exists to prevent.
