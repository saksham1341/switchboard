...
| scratchpad | `actuator/kv/` `session:<sid>:*` | dies with session (same TTL) | `scratchpad` tool — **built** |
...
- **Scratchpad** — the decider injects `ttl=IDLE_TTL` (the session's own sliding TTL) on `scratchpad` kv commands. **Long-term memory** — no ttl.
