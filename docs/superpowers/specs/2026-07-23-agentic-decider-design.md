...
| scratchpad | `actuator/kv/` `session:<sid>:*` | same TTL *duration* as the session, but stamped at write and **not** sliding (§7.3) | `scratchpad` tool — **built** |
...
**The TTL slides.** It is refreshed on every write, not fixed from session birth. Each turn rewrites the whole record anyway, so `store.set(..., ttl=IDLE_TTL)` gives it for free. Absolute expiry would drop memory mid-conversation on a long active thread — precisely backwards. Concretely, `Sessions.save()` (`switchboard/deciders/agent/session.py`) writes **two** keys on every call — `session:<sid>` (state, messages, buffer, gather, `busy_since`) and the `thread:<source>:<key>` route that maps an incoming message back to `sid` — both with the same TTL. It refreshes both, not only the session record, because they must expire *together*: if only one slid, a message could resolve through a route pointing at a session that had already (or not yet) expired on its own independent clock, reintroducing exactly the split-expiry failure this section opens with.

- **Scratchpad** — the decider injects `ttl=IDLE_TTL` (the same duration as the session's TTL) on `scratchpad` kv commands. That duration is stamped at write and does **not** slide with the session — see §7.3. **Long-term memory** — no ttl.
- **`/reset`** — the `discord.command.reset` observation is handled by `AgentDecider._on_reset`, which deletes the session record; next message starts clean. It always acknowledges, even when the channel has no session, so the slash command never looks like it silently did nothing.
...
**A scratchpad entry's TTL is stamped at write and does not slide.** The session record slides — `Sessions.save()` rewrites it every turn (§6.4) — but a scratchpad entry is written once, by one `kv {op:set, ttl:IDLE_TTL}` command, and nothing rewrites it afterwards. Its clock therefore runs from the moment it was stored, independently of the conversation it belongs to. A note written early in a long ACTIVE conversation can expire while that conversation is still live: the session keeps sliding past the TTL on every turn, the note does not, and the next `scratchpad {op:get}` returns `None` for a key the model correctly remembers writing. This is the actual behaviour, stated rather than implied by "dies with the session" — whether the entry's TTL should slide with its session is an open design decision, not something the code currently does.
...
| session TTL (`SB_SESSION_TTL_S`, sliding, §6.4), `/reset`, stuck-busy watchdog (`SB_STUCK_MARGIN`, §6.4), scratchpad/memory tools (§7.3) | decider | **built** (Phase 5) |
