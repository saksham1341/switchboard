...
def test_a_scratchpad_set_injects_no_ttl_at_all():
    """Inverted deliberately. A scratchpad entry used to carry the session's TTL
    duration, stamped at write and not sliding, so a note taken early in a long
    live conversation expired underneath it. Expiry is now the decider's: it
    ENDS a session (on /reset, or on idle expiry seen at clock.tick) and drains
    `session:<sid>:` as part of ending it. The entry itself is immortal until
    that drain, which is what actually makes the tool's promise true."""
    args = rewrite("scratchpad", {"op": "set", "key": "draft", "value": "x"},
                   sid=100)
...
def test_list_is_scoped_to_the_namespace_not_the_whole_store():
    args = rewrite("scratchpad", {"op": "list"}, sid=100)
    assert args["prefix"] == "session:100:"
    args = rewrite("memory", {"op": "list"}, sid=100)
...
def test_a_non_memory_tool_is_not_ours():
    assert rewrite("discord.post", {"content": "hi"}, sid=100) is None
